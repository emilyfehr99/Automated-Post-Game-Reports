...
interface SimpleDrillCreatorProps {
  ...
  onClose: () => void
  editDrill?: any // Optional drill to edit
}

export default function SimpleDrillCreator({ onSave, onClose, editDrill }: SimpleDrillCreatorProps) {
  const [drillName, setDrillName] = useState(editDrill?.name || '')
  const [drillDescription, setDrillDescription] = useState(editDrill?.description || '')
  const [drillCategory, setDrillCategory] = useState(editDrill?.category || 'Skills')
  const [uploadedImages, setUploadedImages] = useState<string[]>(editDrill?.images || [])
  const [setupSteps, setSetupSteps] = useState<string[]>(editDrill?.setup?.length ? editDrill.setup : [''])
  const [variations, setVariations] = useState<string[]>(editDrill?.variations?.length ? editDrill.variations : [''])
  const [coachingPoints, setCoachingPoints] = useState<string[]>(editDrill?.coachingPoints?.length ? editDrill.coachingPoints : [''])
  const [customCategories, setCustomCategories] = useState<string[]>([])
  const [showCategoryInput, setShowCategoryInput] = useState(false)
  const [newCategory, setNewCategory] = useState('')

  const defaultCategories = ['Skills', 'Systems', 'Offensive', 'Defensive', 'Power Play', 'Penalty Kill', 'Warm-up', 'Cool-down']
  const allCategories = [...defaultCategories, ...customCategories]

  // Load custom categories from localStorage on component mount
  useEffect(() => {
    const saved = localStorage.getItem('hockey-custom-categories')
    if (saved) {
  ...
  useEffect(() => {
    localStorage.setItem('hockey-custom-categories', JSON.stringify(customCategories))
  }, [customCategories])
  ...
  const addCustomCategory = () => {
    if (newCategory.trim() && !allCategories.includes(newCategory.trim())) {
      ...
      setDrillCategory(newCategory.trim())
      setNewCategory('')
  ...
  const generateDrillPlan = async () => {
    ...
    const drillData = {
      id: editDrill?.id || Date.now().toString(),
      name: drillName,
      ...
      createdAt: editDrill?.createdAt || new Date().toISOString()
    }
      ...
      >
        ...
        <div className="bg-hockey-blue text-white p-6">
          <div className="flex justify-between items-center">
            <h2 className="text-2xl font-russo">{editDrill ? 'Edit Drill' : 'Simple Drill Creator'}</h2>
            <button
              onClick={onClose}
          ...
          <div className="space-y-6">
                ...
                {!showCategoryInput ? (
                  ...
                  <div className="flex space-x-2">
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-hockey-blue text-white rounded-lg hover:bg-blue-800 transition-colors"
                    >
                      Add
                    </button>
                    <button
            ...
            <div className="flex justify-end space-x-4 pt-6 border-t">
              ...
              >
                ...
                {editDrill ? 'Save Changes' : 'Create Drill Plan'}
              </button>
            </div>
          </div>
        </div>
