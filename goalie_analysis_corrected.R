...
# Check slot shots (opposing team) - using Shots against
slot_shots <- goalie_data %>%
  filter(action == "Shots against" & team != goalie_team & is_slot(pos_x, pos_y))
cat("Number of Shots against by opposing team in slot (|pos_x| < 30, |pos_y| < 15):", nrow(slot_shots), "\n")
if (nrow(slot_shots) > 0) {
  ...
  cat("Warning: No shots in slot by opposing team. Check pos_x/pos_y ranges or slot definition.\n")
}
...
# Save Percentage After Pass
# NOTE: This may have limited data since 'Passes to the slot' is not present
...
cat("Number of Shots against by opposing team following Passes within 5 actions:", nrow(pass_sequences), "\n")
...
# Using Shots against instead of Shots
...
# 2. Workload Metrics
# Shot Suppression Rate - using Shots against
...
print("Shot Suppression Rate by Half (Shots against by opposing team):")
...
# High-Danger Shots Faced - using Shots against
...
print("High-Danger Shots Faced (opposing team):")
print(high_danger_shots_faced)

# Shot Location Metrics - using Shots against
...
print("Shot Location Metrics (Average and Variance, opposing team):")
...
cat("2. Missing actions: 'Power play shots', 'Short-handed shots', 'Passes to the slot'\n")
cat("3. Duration is constant (12) for all entries\n")
cat("4. High-danger shots in slot area: ", nrow(slot_shots), "\n")
cat("5. Total goalies analyzed: ", length(goalies), "\n")
cat("6. Overall save percentage range: ", round(min(overall_save_pct$save_pct, na.rm = TRUE), 2), "% to ", round(max(overall_save_pct$save_pct, na.rm = TRUE), 2), "%\n")
