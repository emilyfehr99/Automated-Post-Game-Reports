#!/usr/bin/env python3
"""
Russo One Font Goalie Analytics Dashboard
Clean, professional design with Russo One font throughout
"""
...
def create_russo_one_dashboard():
    # Load the data
    ...
    print("=== CREATING RUSSO ONE DASHBOARD ===")
    ...
    # Set up the figure with Russo One font
    plt.rcParams['font.family'] = 'Russo One'
    ...
    ax_title.text(0.5, 0.6, 'GOALIE ANALYTICS DASHBOARD', 
                  fontsize=36, fontweight='normal', ha='center', va='center',
    ...
    bars1 = ax1.bar(x - width/2, distance_df['close_save_pct'], width, 
                    label='CLOSE RANGE (<15 UNITS)', color=colors['accent'], alpha=0.9)
    bars2 = ax1.bar(x + width/2, distance_df['far_save_pct'], width, 
                    label='FAR RANGE (≥15 UNITS)', color=colors['info'], alpha=0.9)
    
    # Add value labels on bars
    for bar in bars1:
        height = bar.get_height()
        ax1.text(bar.get_x() + bar.get_width()/2., height + 1,
                f'{height:.1f}%', ha='center', va='bottom', fontweight='normal', fontsize=10)
    ...
    ax1.set_title('SAVE % BY SHOT DISTANCE', fontsize=16, fontweight='normal', color=colors['primary'])
    ax1.set_ylabel('SAVE PERCENTAGE (%)', fontweight='normal', fontsize=12)
    ax1.set_xticks(x)
    ax1.set_xticklabels([name.split()[0] for name in distance_df['goalie']], fontweight='normal', fontsize=11)
    ...
    bars1 = ax2.bar(x - width, angle_df['front_save_pct'], width, 
                    label='FRONT OF NET', color=colors['success'], alpha=0.9)
    bars2 = ax2.bar(x, angle_df['side_save_pct'], width, 
                    label='SIDE ANGLES', color=colors['warning'], alpha=0.9)
    bars3 = ax2.bar(x + width, angle_df['behind_save_pct'], width, 
                    label='BEHIND NET', color=colors['accent'], alpha=0.9)
        ...
        for bar in bars:
            ...
            if height > 0:
                ax2.text(bar.get_x() + bar.get_width()/2., height + 1,
                        f'{height:.1f}%', ha='center', va='bottom', fontweight='normal', fontsize=9)
    
    ax2.set_title('SAVE % BY SHOT ANGLE', fontsize=16, fontweight='normal', color=colors['primary'])
    ax2.set_ylabel('SAVE PERCENTAGE (%)', fontweight='normal', fontsize=12)
    ax2.set_xticks(x)
    ax2.set_xticklabels([name.split()[0] for name in angle_df['goalie']], fontweight='normal', fontsize=11)
    ...
    ax3.set_xlabel('SHOT DISTANCE FROM NET', fontweight='normal', fontsize=12)
    ax3.set_ylabel('DENSITY', fontweight='normal', fontsize=12)
    ax3.set_title('SHOT DISTANCE DISTRIBUTION', fontsize=16, fontweight='normal', color=colors['primary'])
    ...
    ax4.text(net_x, net_y, 'NET', ha='center', va='center', color='white', 
             fontweight='normal', fontsize=10, zorder=11)
    
    ax4.set_xlabel('X POSITION', fontweight='normal', fontsize=12)
    ax4.set_ylabel('Y POSITION', fontweight='normal', fontsize=12)
    ax4.set_title('SAVE % HEAT MAP', fontsize=16, fontweight='normal', color=colors['primary'])
    ax4.grid(True, alpha=0.3)
    
    # Add colorbar
    cbar = plt.colorbar(im, ax=ax4, shrink=0.8)
    cbar.set_label('SAVE %', fontweight='normal', fontsize=10)
    ...
    for i, goalie in enumerate(goalies):
        ...
        for _, shot in sample_shots.iterrows():
            ax5.plot([shot['pos_x'], net_x], [shot['pos_y'], net_y], 
                    color=goalie_colors[i % len(goalie_colors)], alpha=0.4, linewidth=1.5, zorder=1)
    ...
    ax5.set_xlabel('X POSITION', fontweight='normal', fontsize=12)
    ax5.set_ylabel('Y POSITION', fontweight='normal', fontsize=12)
    ax5.set_title('SHOT LOCATIONS & TRAJECTORIES', fontsize=16, fontweight='normal', color=colors['primary'])
    ax5.legend(bbox_to_anchor=(1.05, 1), loc='upper left', frameon=True, fancybox=False, shadow=False, fontsize=9)
    ...
    for i, data in enumerate(performance_data):
        # Card background
        card = Rectangle((0.05, y_pos - 0.12), 0.9, 0.1, 
                        facecolor=goalie_colors[i % len(goalie_colors)], 
                        alpha=0.1, transform=ax6.transAxes)
        ax6.add_patch(card)
        
        # Goalie name
        ax6.text(0.1, y_pos, f"{i+1}. {data['goalie']}", 
                fontsize=14, fontweight='normal', color=colors['primary'], transform=ax6.transAxes)
        
        # Stats
        ax6.text(0.1, y_pos - 0.04, f"SAVE %: {data['save_pct']:.1f}% ({data['saves']}/{data['saves']+data['goals']})", 
                fontsize=11, color=colors['secondary'], transform=ax6.transAxes)
        ax6.text(0.1, y_pos - 0.08, f"AVG DISTANCE: {data['avg_distance']:.1f} UNITS", 
                fontsize=11, color=colors['secondary'], transform=ax6.transAxes)
    ...
    bars1 = ax7.bar(x - width, zone_df['defensive_zone'], width, 
                     label='DEFENSIVE ZONE', color=colors['success'], alpha=0.9)
    bars2 = ax7.bar(x, zone_df['neutral_zone'], width, 
                     label='NEUTRAL ZONE', color=colors['warning'], alpha=0.9)
    bars3 = ax7.bar(x + width, zone_df['offensive_zone'], width, 
                     label='OFFENSIVE ZONE', color=colors['info'], alpha=0.9)
    ...
    ax7.set_title('SAVE % BY ZONE', fontsize=16, fontweight='normal', color=colors['primary'])
    ax7.set_ylabel('SAVE PERCENTAGE (%)', fontweight='normal', fontsize=12)
    ax7.set_xticks(x)
    ax7.set_xticklabels([name.split()[0] for name in zone_df['goalie']], fontweight='normal', fontsize=11)
    ax7.legend(frameon=True, fancybox=False, shadow=False, fontsize=10)
    ...
    bars1 = ax8.bar(x - width/2, pass_df['pass_save_pct'], width, 
                    label='AFTER PASS', color=colors['accent'], alpha=0.9)
    bars2 = ax8.bar(x + width/2, pass_df['overall_save_pct'], width, 
                    label='OVERALL', color=colors['secondary'], alpha=0.9)
    ...
    ax8.set_title('SAVE % AFTER PASS', fontsize=16, fontweight='normal', color=colors['primary'])
    ax8.set_ylabel('SAVE PERCENTAGE (%)', fontweight='normal', fontsize=12)
    ax8.set_xticks(x)
    ax8.set_xticklabels([name.split()[0] for name in pass_df['goalie']], fontweight='normal', fontsize=11)
    ax8.legend(frameon=True, fancybox=False, shadow=False, fontsize=10)
    ...
    ax9.text(0.05, 0.9, insight_text, transform=ax9.transAxes, fontsize=12,
              verticalalignment='top', fontfamily='Russo One', fontweight='normal',
              color=colors['primary'],
              bbox=dict(boxstyle="round,pad=0.5", facecolor=colors['light_gray'], 
                       edgecolor=colors['primary'], linewidth=2, alpha=0.9))
    
    # Save the dashboard
    plt.savefig('/Users/emilyfehr8/CascadeProjects/russo_one_dashboard.png', 
                dpi=300, bbox_inches='tight', facecolor=fig.get_facecolor())
    print("Russo One dashboard saved as 'russo_one_dashboard.png'")
    
    return goalie_data

if __name__ == "__main__":
    goalie_data = create_russo_one_dashboard()
